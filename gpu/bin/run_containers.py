...
def launch_container(clipper, model_name, model_version, model_input_type,
                     clipper_ip, gpu_mem_frac, checkpoint_path, tf_slim_path):
    ...
    add_container_cmd = (
        ...
        "{image}".format(
            ...
            mv_label="%s=%s:%s" % (CLIPPER_MODEL_CONTAINER_LABEL, model_name,
                                   model_version),
            restart_policy='no'))
...
if __name__ == "__main__":
    ...
    for i in range(0, num_containers):
        print("Launched container {}?: {}".format(
            i,
            launch_container(clipper, name, version, input_type, clipper_ip,
                             gpu_mem_frac, checkpoint_path, tf_slim_path)))
