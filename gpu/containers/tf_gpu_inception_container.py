...
class TfInceptionContainer(rpc.ModelContainerBase):
    def __init__(self, checkpoint_path, gpu_mem_frac):
        ...
        self.sess = tf.Session(
            "", tf.Graph(), config=tf.ConfigProto(gpu_options=gpu_options))
        with self.sess.graph.as_default():
            self.inputs = tf.placeholder(tf.float32, (None, image_size,
                                                      image_size, 3))
    ...
    def predict_bytes(self, inputs):
        ...
        with self.sess.graph.as_default():
            decoded_images = [
                tf.image.decode_jpeg(input_image.tobytes(), channels=3)
                for input_image in inputs
            ]
            preprocessed_images = [
                inception_preprocessing.preprocess_image(
                    decoded_image, image_size, image_size, is_training=False)
        ...
        all_probabilities = self.sess.run(
            ...
            feed_dict={self.inputs: processed_inputs})
