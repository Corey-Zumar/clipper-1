...
class Predictor(object):
    ...
    def print_stats(self):
        ...
        self.stats["probabilities"].append((nmt_ptc, lstm_pct, ld_pct))
...
if __name__ == "__main__":
    ...
    all_thru_batches = (31, 21, 16)

    latency_upper_bound = 10.0
    ...
    for i in range(len(all_reps)):
        ...
        configs = [
            setup_lang_detect(batch_size=all_thru_batches[lang_detect_batch_idx],
                              num_replicas=lang_detect_reps,
                              cpus_per_replica=1,
                              allocated_cpus=get_cpus(lang_detect_reps),
                              allocated_gpus=[],
                              input_size=input_size),
            setup_nmt(batch_size=all_thru_batches[nmt_batch_idx],
                      num_replicas=nmt_reps,
                      cpus_per_replica=1,
                      allocated_cpus=get_cpus(nmt_reps),
                      allocated_gpus=get_gpus(nmt_reps),
                      input_size=input_size),
            setup_lstm(batch_size=all_thru_batches[lstm_batch_idx],
        ...
        benchmarker = DriverBenchmarker(configs, queue, client_num, latency_upper_bound, input_size, request_delay)
